        ...
        if (Cache::HaveFreeWritingSlots())
        {
            ...
            for (int i =0 ; i < m_ram.m_numBlocks; ++i) 
            {
                ...
                if (m_ram.m_blockStates[i].refCount == 0 && m_ram.m_blockStates[i].fileBlockIdx != t.fileBlockIdx )
                {
                    ...
                    m_ram.m_blockStates[i].fileBlockIdx = t.fileBlockIdx;
                    break;
                }
            }
...
{ 
   m_downloadStatusMutex.Lock();
   bool already = m_cfi.TestBit(task->fileBlockIdx);
   m_downloadStatusMutex.UnLock();
...
Prefetch::WriteBlockToDisk(int ramIdx, int fileIdx, size_t size)
{
   ...
   int retval = 0;
   /*
//AMT don't need it still has ref count 1 from creation in readfrom task
   m_ram.m_writeMutex.Lock();
...
//______________________________________________________________________________
bool Prefetch::FreeRamBlock(int ramIdx)
{
   bool released = false;
