   ...
   if (m_oss->Stat(i_name.c_str(), &sbuff) == XrdOssOK)
   {
      ...
      if (GetCacheControlXAttr(i_name, icc) > 0) {
         ...
         if (cc_json.contains("ETag") && (mustRevalidate || hasExpired)) {
            ...
            std::string response;
            std::string fsctlarg = curl;
            fsctlarg.push_back('\0');
            fsctlarg += "head"; // add subcommand for file system query
            int st = XrdPosixExtra::FSctl(XrdOucCacheOp::Code::QFSinfo, fsctlarg, response, false); 
            
            if (st == 0)
            {
               std::string etag = response;
            ...
            {
               // Message has a status because we are in the block condition for cache-contol xattr
